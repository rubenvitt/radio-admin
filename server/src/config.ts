...
// must be ≥16 chars; under dev bypass we fall back to a fixed throwaway value so
// the local/all-in-one server boots without configuring OIDC. A value supplied
// under dev bypass is still validated for length (a too-short secret is a bug,
// not a convenience).
...
const configSchema = z
  .object({
    ...
    SESSION_SECRET: z
      .string()
      .min(SESSION_SECRET_MIN, 'SESSION_SECRET must be at least 16 characters')
      .optional(),
